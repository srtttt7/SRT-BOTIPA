...
# 2. الإعدادات
TOKEN = os.environ.get("BOT_TOKEN", "8686601094:AAEViStOO6vokqRvEDnFY8Wj2LwtY0eqKHc")
...
def make_direct_ota(ipa_url: str, bundle_id: str, app_version: str, app_name: str, user_dir: str):
...
async def process_p12(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc or not file_name.endswith('.p12'):
        await update.message.reply_text("❌ هذا ليس ملف `.p12` صحيح، يرجى إعادة الإرسال.")
        return WAITING_P12
...
async def process_prov(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc or not (file_name.endswith('.mobileprovision') or file_name.endswith('.provision')):
        await update.message.reply_text("❌ هذا ليس ملف `.mobileprovision` صحيح، يرجى إعادة الإرسال.")
        return WAITING_PROV
...
async def process_pass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🎉 تم حفظ الشهادة بنجاح! اضغط على \"توقيع تطبيق IPA\" وأرسل تطبيقك.")
    return ConversationHandler.END
...
async def handle_ipa_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التوقيع المباشر عبر pyzsign بدون أوامر خارجية
    try:
        signer = Signer(
            p12_file=p12_path,
            password=p12_pass,
            mobileprovision=prov_path
        )
        signer.sign(input_ipa, output_ipa)
        success = os.path.exists(output_ipa)
    except Exception as e:
    ...
    if success:
        ...
        if ipa_download_url:
            install_button_url, _ = make_direct_ota(ipa_download_url, bundle_id, app_version, app_name, user_dir)
        ...
        await status_msg.edit_text("❌ حدث خطأ أثناء التوقيع. تأكد من صحة ملفات الشهادة وكلمة السر.")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "check_cert":
        p12_exists = "✅ موجود" if os.path.exists(os.path.join(user_dir, "cert.p12")) else "❌ غير موجود"
        prov_exists = "✅ موجود" if os.path.exists(os.path.join(user_dir, "cert.mobileprovision")) else "❌ غير موجود"
        pass_exists = "✅ مضافة" if os.path.exists(os.path.join(user_dir, "pass.txt")) else "❌ غير مضافة"
        ...
        if deleted:
            await query.message.reply_text("🗑️ **تم حذف جميع ملفات الشهادة بنجاح.**", parse_mode='Markdown')
        else:
...
if __name__ == '__main__':
    ...
    sign_handler = ConversationHandler(
        entry_points=[CallbackQueryHandler(start_sign_flow, pattern='^start_sign$')],
        states={
